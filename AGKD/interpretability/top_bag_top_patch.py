...
from datasets.datasetsC16 import MyDataset, Data_embedding
from model.ResNetb import MRAN
...
parser.add_argument('--output_dir', type=str, default='/home/zhouyike/AGKD/Han/AGKD/MRAN_split_3/output', help='directory to save csv file')
...
#bag vari:1,10,20,30,40,50,60
parser.add_argument('--top_bag_num', type=int, default=30, help='select the top_bag_num bags with the highest attention scores')# set patch 30
parser.add_argument('--top_patch_num', type=int, default=60, help='select the top_patch_num patches with the highest attention scores in each selected bag')
...
def evaluate(test_loader, model, epoch):
    ...
    with torch.no_grad():
        for i, (img, wsi_id, bag_id, label, bag_path, bag_weight, index,wsi_name,patch_alpha) in enumerate(
            ...
            img = img.cuda()
            label = label.cuda()
            patch_alpha = patch_alpha.cuda()  # Make sure patch_alpha is passed and used

            with autocast():
                y, embedding, alpha, y_alpha, beta, y_beta = model(img, patch_alpha=patch_alpha, tag=0)  # y: n*2 embedding : n * 512 alpha : n * 64
...
def test(model):
    ...
    test_loader = DataLoader(dataset=test_data, batch_size=1, num_workers=8, drop_last=True, shuffle=False, pin_memory=True, prefetch_factor=8)
...
def make_csv():
    ...
    test0 = pd.read_csv(f'{init_csv_dir}/case_test_inter.csv')
    tot = test0
    ...
    # /home/zhouyike/AGKD/Han/AGKD/WSI/slides/images
    # /home/zhouyike/AGKD/Han/AGKD/WSI/slides/images/normal_043.tif
    # print(tot, 'test_Wsi',len(test_wsi),'test_wsi_bag',len(test_wsi_bag), 'best_wsi_alpha',len(a['best_wsi_alpha']))
    # print(test_wsi,tot['wsi'][:60])
...
if __name__ == '__main__':
    ...
    if torch.cuda.is_available():
        ...
        net.to(device)  # Load the model onto GPU 0
        net.upstream = torch.nn.DataParallel(net.upstream, device_ids=[0,1]) 
        net.downstream = torch.nn.DataParallel(net.downstream, device_ids=[0,1]) 
