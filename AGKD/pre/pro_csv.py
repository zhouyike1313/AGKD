...
parser.add_argument('--total_sheet_path', type=str, default='csv-stad/example/sheet/total.csv', help='path to total sheet file')
parser.add_argument('--sheet_dir', type=str, default='csv-stad/example/sheet', help='directory to save sheet file')
parser.add_argument('--split_dir', type=str, default='csv-stad/example/split', help='directory to save split csv file')
parser.add_argument('--loc_path', type=str, default='csv-stad/example/loc/', help='path to loc csv file')
parser.add_argument('--wsi_dir', type=str, default='WSI/example', help='path to svs file')

parser.add_argument('--s_train', type=int, default=6, help='proportion of training set')
parser.add_argument('--s_val', type=int, default=2, help='proportion of validation set')
parser.add_argument('--s_test', type=int, default=2, help='proportion of test set')
parser.add_argument('--k', type=int, default=5, help='k-fold Monte Carlo cross-validation')
...
def make_case_sheet_abc(s_train, s_val, s_test, dir_id=0):
    ...
    tot = list(tot_sheet['File Name'])
    tot = list(set([i[:12] for i in list(tot_sheet['File Name'])]))
    shuffle(tot)

    cnt_train = math.floor(len(tot) * s_train / (s_train + s_val + s_test))
    cnt_eval = math.floor(len(tot) * s_val / (s_train + s_val + s_test))

    case_train = []
    case_val = []
    case_test = []
    for cnt, i in enumerate(tot):
        if cnt < cnt_train:
            case_train.append(i)
        elif cnt < cnt_train + cnt_eval:
            case_val.append(i)
        else:
            case_test.append(i)
        cnt += 1
    ...
    for i in range(len(tot_sheet)):
        ...
        if t[0] in case_train:
            n_train.loc[len(n_train) + 1] = t
        elif t[0] in case_val:
