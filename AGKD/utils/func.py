...
def weight_cross_entropy(y, label, w, reduction='none', loss=None):
    #print(y.shape, label.shape, w.shape)
    if loss is None:
...
def compute_loss2(alpha,y_alpha):
    # y_alpha = F.softmax(y_alpha, dim=1)
    # y_alpha = torch.max(y_alpha,dim=1).values / torch.sum(y_alpha,dim=1)
    # loss2 = torch.norm(alpha.flatten()-y_alpha,p=2,dim=0)
    loss2 = torch.min(torch.tensor([torch.square(alpha[i] - torch.max(y_alpha[i])/torch.sum(y_alpha[i])) for i in range(alpha.shape[0])]))
    return loss2
